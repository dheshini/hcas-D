...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST['username']))) {
        ...
        if (!preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[^A-Za-z\d\s])\S{6,}$/', $username)) {
            $username_err = '<span class="error">Username should contain at least one letter, one symbol, one number, and be at least 6 characters long.</span>';
        } else {
        ...
        if ($username === $password) {
            $username_err = '<span class="error">Username should not be the same as the password.</span>';
        }
    }
    // Validate email
...
if (empty(trim($_POST['password']))) {
    ...
    $password_err = '<span style="color: red;">Password must be at least 6 characters long.</span>';
} else {
    $password = trim($_POST['password']);
    // Password strength validation
    if (!preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()_+}{":;\'?\/><.,|`~])(?=.*[^\w\d\s]).{8,}$/', $password)) {
        $password_err = '<span style="color: red;">Password must contain at least 8 characters with at least one uppercase letter, one lowercase letter, one number, and one special character.</span>';
    }
}

    // Validate confirm password
    if (empty(trim($_POST['cpassword']))) {
        $cpassword_err = '</span>Please confirm your password.</span>';
    } else {
        $cpassword = trim($_POST['cpassword']);
        if ($password != $cpassword) {
            $cpassword_err = '<span style="color: red;">Passwords do not match.</span>';
        }
    }
...
if (empty($username_err) && empty($email_err) && empty($phone_err) && empty($password_err) && empty($cpassword_err)) {
    ...
    // Set expiration time to 2 minutes from now
        $expiration_time = strtotime('+2 minutes');
...
$message .= '<p>Your verification code will expire in 12 Hours. Please click the button below to verify your email address:</p>';$message .= '<a href="http://localhost/Hannani%20Project/user/verify.php?code='.$verification_code.'" style="background-color: #ff652f; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block;">Verify Email</a>';
$message .= '</td>';
$message .= '</tr>';
$message .= '<tr>';
$message .= '<td style="padding: 20px; text-align: center; background-color: #f1f1f1;">';
...
if (isset($_GET['code'])) {
    ...
    $sql_check_code = "SELECT verification_code_expires FROM patients WHERE verification_code = ?";
